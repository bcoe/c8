...
class Report {
  ...
  _getSourceMap (v8ScriptCov) {
    ...
    if (!sourceMapAndLineLengths) return sources

    // See: https://github.com/nodejs/node/pull/34305
    if (!sourceMapAndLineLengths.data) return
    sources.sourceMap = { sourcemap: sourceMapAndLineLengths.data }

    if (!sourceMapAndLineLengths.lineLengths) return sources
  ...
  _getMergedProcessCov () {
    ...
    for (const v8ProcessCov of reports) {
      if (Array.isArray(v8ProcessCov.result)) {
    ...
    if (!this.all) return mergeProcessCovs(v8ProcessCovs)

    const emptyReports = []
    v8ProcessCovs.unshift({ result: emptyReports })

    for (const workingDir of this.src) {
      const excludes = this.exclude.globSync(workingDir)
      for (const file of excludes) {
        const fullPath = resolve(workingDir, file)
        if (fileIndex.has(fullPath)) continue
  ...
  _loadReports () {
    ...
    for (const file of files) {
      try {
        const content = readFileSync(resolve(this.tempDirectory, file), 'utf8')
  ...
  _normalizeProcessCov (v8ProcessCov, fileIndex) {
    ...
    for (const v8ScriptCov of v8ProcessCov.result) {
      ...
      if ((!this.omitRelative || isAbsolute(v8ScriptCov.url)) &&
          (this.excludeAfterRemap || this.exclude.shouldInstrument(v8ScriptCov.url))) {
  ...
  _normalizeSourceMapCache (v8SourceMapCache) {
    const cache = {}
    for (const [fileURL, value] of Object.entries(v8SourceMapCache)) {
      cache[pathToFileURL(fileURLToPath(fileURL)).href] = value
    }
