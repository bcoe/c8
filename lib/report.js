...
class Report {
  ...
  async getMonocart () {
    let MCR
    try {
      MCR = await import('monocart-coverage-reports')
    } catch (e) {
      console.error('ERROR: Failed to import module "monocart-coverage-reports". Please try to install it: npm i monocart-coverage-reports --save-dev')
      process.exit(1)
    }
    return MCR
  }
